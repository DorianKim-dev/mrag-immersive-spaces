...
const PressPage = () => {
  ...
  const handleArticlePointer = (index: number, event: PointerEvent<HTMLElement>) => {
  ...
  const previewLeft = Math.max(16, Math.min(mousePos.x + 24, viewport.width - 256));
  const previewTop = Math.max(16, Math.min(mousePos.y + 24, viewport.height - 176));
  const hoveredArticle = hoveredIndex !== null ? visibleArticles[hoveredIndex] : null;
    ...
    <>
      ...
      {hoveredArticle
        ? createPortal(
            <AnimatePresence>
              <motion.div
                className="pointer-events-none fixed z-[1000] h-[160px] w-[240px] overflow-hidden border border-mrag-teal/20 bg-mrag-navy shadow-[0_22px_80px_rgba(0,0,0,0.44)]"
              ...
              >
                <img
                  src={PRESS_PREVIEW_IMAGES[hoveredArticle.previewIdx]}
                  alt=""
              ...
              {visibleArticles.map((article, i) => {
                const inner = (
                  <motion.article
                    ...
                    onPointerEnter={(event) => handleArticlePointer(i, event)}
                    onPointerMove={(event) => handleArticlePointer(i, event)}
